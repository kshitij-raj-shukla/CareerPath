...
export async function updateProfile(token: string, payload: ProfileUpdatePayload): Promise<UserProfile> {
...
export async function submitAssessment(payload: AssessmentRequest, token?: string | null): Promise<AssessmentResponse> {
  return request<AssessmentResponse>("/api/assessment", {
    method: "POST",
    body: JSON.stringify(payload),
  }, token);
}
...
export async function createCareerPlan(payload: CareerPlanRequest, token?: string | null): Promise<CareerPlanResponse> {
  return request<CareerPlanResponse>("/api/career-plan", {
    method: "POST",
    body: JSON.stringify(payload),
  }, token);
}
...
export async function updateProgressTask(planId: string, taskId: number, done: boolean, token: string): Promise<{ ok: boolean }> {
  return request<{ ok: boolean }>(`/api/progress/${planId}/task`, {
    method: "PATCH",
